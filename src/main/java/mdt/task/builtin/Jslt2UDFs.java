...
public class Jslt2UDFs {
	...
	static class TraverseSME implements Jslt2Function {
		...
		public JsonNode execute(JsonNode input, JsonNode... args) throws Jslt2Exception {
			...
			if ( args[0].isObject() ) {
				try {
					SubmodelElement top = AASUtils.readJson(args[0], SubmodelElement.class);
					SubmodelElement found = SubmodelUtils.traverse(top, path);
					String outputStr =  AASUtils.writeJson(found);
					return AASUtils.getJsonMapper().readTree(outputStr);
				}
				catch ( Exception e ) {
			...
			if ( args[0].isObject() ) {
				try {
					...
					if ( found instanceof Property ) {
						String outputStr =  AASUtils.writeJson(found);
						JsonNode target = AASUtils.getJsonMapper().readTree(outputStr);
	...
	public static final void main(String... args) throws Exception {
		...
		JsonNode root = AASUtils.getJsonMapper().readTree(new File("misc/test-sme.json"));
//		String code = """
